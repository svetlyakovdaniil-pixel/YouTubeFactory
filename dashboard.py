...
with tab_vod:
    ...
    for day in WEEK_DAYS:
        ...
        with st.expander(
            ...
            st.markdown("#### Генератор описания")
            st.caption(
                "При публикации система случайно выберет "
                "по одному варианту из каждого блока. "
                "Один вариант — одна строка."
            )

            description_blocks = item.get(
                "description_blocks",
                {},
            )
            ...
            intro_text = st.text_area(
                "Вступления",
                value="\n".join(
                    description_blocks.get("intro", [])
                ),
                height=110,
                key=(
                    f"vod_description_intro_"
                    f"{selected_channel}_{day}"
                ),
            )
            genre_text = st.text_area(
                "Описание звучания и поджанра",
                value="\n".join(
                    description_blocks.get("genre", [])
                ),
                height=130,
                key=(
                    f"vod_description_genre_"
                    f"{selected_channel}_{day}"
                ),
            )
            usage_text = st.text_area(
                "Для чего подходит",
                value="\n".join(
                    description_blocks.get("usage", [])
                ),
                height=110,
                key=(
                    f"vod_description_usage_"
                    f"{selected_channel}_{day}"
                ),
            )
            cta_text = st.text_area(
                "Призыв к действию",
                value="\n".join(
                    description_blocks.get("cta", [])
                ),
                height=100,
                key=(
                    f"vod_description_cta_"
                    f"{selected_channel}_{day}"
                ),
            )
            hashtags_text = st.text_area(
                "Хэштеги",
                value="\n".join(
                    description_blocks.get("hashtags", [])
                ),
                height=100,
                key=(
                    f"vod_description_hashtags_"
                    f"{selected_channel}_{day}"
                ),
            )

            description_text = st.text_area(
                "Старое цельное описание (резерв)",
                value=item.get(
                ...
                height=100,
                help=(
                    "Используется только если блоки выше "
                    "пустые. {tracklist} можно оставить "
                    "для обратной совместимости."
                ),
                key=(
                    f"vod_description_legacy_"
                    f"{selected_channel}_{day}"
                ),
            )
            ...
            if st.button(
                ...
                values = {
                    ...
                    "description_blocks": {
                        ...
                        "genre": [
                            line.strip()
                            for line in genre_text.splitlines()
                            if line.strip()
                        ],
                        "usage": [
                            line.strip()
                        ...
                        "cta": [
                            line.strip()
                            for line in cta_text.splitlines()
                            if line.strip()
                        ],
                        "hashtags": [
                            line.strip()
